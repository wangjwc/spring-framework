...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			// 准备刷新所用的上下文环境（比如对系统属性和环境变量进行准备和验证）
			// Prepare this context for refreshing.
			prepareRefresh();

			// 初始化BeanFactory，并加载BeanDefinition（比如并进行xml文件读取）
			...
			try {
				 ...
				 *
				 * 特殊的：
				 *    1、AutowiredAnnotationBeanPostProcessor：用于解析bean的构造方法参数列表
				 * 		注意点：
				 * 			假如某个bean在该处理器注册前实例化，且没有无参构造函数时，会实例化失败
				 * 			（比如，在@Configuration类中注册了一个BeanFactoryPostProcessor，
				 * 				或者注册了BeanPostProcessor且优先级比AutowiredAnnotationBeanPostProcessor高时）
				 *
				 * 使用SpringBoot时，关注以下方法中注册的注解
				 *  @see org.springframework.context.annotation.AnnotationConfigUtils#registerAnnotationConfigProcessors
				 */
				// Register bean processors that intercept bean creation.
				registerBeanPostProcessors(beanFactory);

				// 注册Message（即用于国际化处理的各种语言消息体）
				...
				// 留给子类初始化其他bean（protect空方法，由子类实现）
				...
				/*
				 * 初始化非延迟加载单例（即实例化所有非lazy的，且在之前步骤中没有初始化的单例bean）
				 * 1、BeanFactory初始化
				 * 2、ConversionService、配置冻结等
				 * 3、not-lazy单例的初始化
				 */
	...
	protected void prepareRefresh() {
		...
		/*
		 * 校验被标注为required的属性（ConfigurablePropertyResolver#setRequiredProperties),
		 * 标注操作可以在initPropertySources中执行
		 * 如果未找到被标注的属性，则抛MissingRequiredPropertiesException异常
		 * 可以用来执行系统必要属性脚要
		 */
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 初始化BeanFactory并进行xml读取, 然后将属性设置到this（this.beanFactory = beanFactory;）
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		// 设置BeanFactory的ClassLoader为Context的ClassLoader
		// Tell the internal bean factory to use the context's class loader etc.
		beanFactory.setBeanClassLoader(getClassLoader());

		/*
		 * 设置BeanFactory的表达式语言处理器(since spring3)
		 * SpEL(Spring Expression Language)，默认可以使用#{bean.xxx}的形式来调用相关属性值
		 * 使用地点参见：
		 * 	@org.springframework.beans.factory.support.AbstractAutowireCapableBeanFactory.applyPropertyValues
		 * 	==> new BeanDefinitionValueResolver(this, beanName, mbd, converter)
		 *    ==> doEvaluate
		 * 	    ==> this.beanFactory.evaluateBeanDefinitionString(value, this.beanDefinition)
		 * 	      ==> this.beanExpressionResolver.evaluate(value, new BeanExpressionContext(this, scope));
		 */
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));

		/*
		 * 为BeanFactory增加一个默认的PropertyEditor（系对bean的属性等设置管理的一个工具）
		 * 比如：将注入属性字符串2020-01-01转为Date类型
		 *
		 * ResourceEditorRegistrar是默认的注册器，支持了class等一些基础类型
		 * （详情见org.springframework.beans.support.ResourceEditorRegistrar#registerCustomEditors）
		 *
		 * 生效时机：
		...
		/*
		 * 注册BeanPostProcessor，如果是实现了ApplicationListener接口的bean，则将其加入到applicationListeners中
		 */
		...
		/*
		 * 默认的系统环境bean：environment、systemProperties、systemEnvironment
		 */
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		/*
		 * 从getBeanFactoryPostProcessors()中和BeanDefinition中查找BeanDefinitionRegistryPostProcessor和BeanFactoryPostProcessor，并执行他们
		 * 其中，BeanDefinitionRegistryPostProcessor extend BeanFactoryPostProcessor
		 * BeanDefinitionRegistryPostProcessor的内部实现类ConfigurationClassPostProcessor，用于支持@Configuration、@Import、@Bean等注解
		 *
		 * getBeanFactoryPostProcessors()： 获取硬编码方式加入到ApplicationContext中的BeanFactoryPostProcessors实现
		 */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		/*
		 * 初始化实现了LoadTimeWeaverAware的Bean
		 *
		 * 核心模块中（context、bean、core）只有AspectJWeavingEnabler实现了LoadTimeWeaverAware接口，用于实现aop静态代理
		 */
